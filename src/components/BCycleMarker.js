...
function BCycleMarker({ station }) {
 ...
 const map =  L.map('map').setView([14.071896310770885, 100.6029928476175], 13);
  ...
  return (
    ...
    eventHandlers={{
      click: () => {
        map.setView([station.lat, station.lon].addTo(map), map.getZoom());
      },
    }}
    >
      ...
      <Popup>
        ...
        <div className="text-end"><a href="http://localhost:3000/" className="text-muted" target={'_blank'} rel={'noreferrer'}>BCycle Nashville</a></div>
      </Popup>
    </Marker>
  );
}
