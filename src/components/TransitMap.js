...
//import CustomMarkers from 'D:\cs266\TUbus\wego-bus-map\src\components\CustomMarker.js';
//import bus_stop from 'D:\cs266\TUbus\wego-bus-map\src\controllers\bus_stop.js';
//import routeMap from 'D:\cs266\TUbus\wego-bus-map\src\controllers\routeMap.js';
...
TransitMap.defaultProps = {
  ...
  alerts: [],
  tripUpdates: [],
  bCycleStations: [14.071896310770885, 100.6029928476175],
  mapControls: {},
  center:  [36.725005, -87.579122], // northwest
  zoom: 12,
  stops: [],
};
...
function TransitMap({
  mapControls, center, zoom,
}) {
  // State สำหรับเก็บข้อมูลจาก JSON
  const [busStops, setBusStops] = useState([]);
  const [routes, setRoutes] = useState([]);

  useEffect(() => {
    // ใช้ fetch หรือวิธีอื่น ๆ ในการดึงข้อมูล
    fetch('D:\cs266\TUbus\wego-bus-map\src\controllers\bus_stop.js') // แทน path-to-bus_stops.json ด้วยที่อยู่ที่ถูกต้อง
      .then((response) => response.json())
      .then((data) => setBusStops(data.busStops))
      .catch((error) => console.error('Error fetching bus stops data:', error));

    fetch('D:\cs266\TUbus\wego-bus-map\src\controllers\routeMap.js') // แทน path-to-routes.json ด้วยที่อยู่ที่ถูกต้อง
      .then((response) => response.json())
  ...
  return (
    <MapContainer className="map-container" center={center} zoom={zoom} scrollWheelZoom={true} maxBounds={cityMaxBounds} doubleClickZoom={false}>
      <TileLayer
        attribution='&copy; <a href="http://www.openstreetmap.org/copyright" target="blank">OpenStreetMap</a> &copy; <a href="http://cartodb.com/attributions" target="blank">CartoDB</a>; <a href="http://www.wegotransit.com" target="blank">WeGo</a>'
        url="https://cartodb-basemaps-{s}.global.ssl.fastly.net/light_all/{z}/{x}/{y}{r}.png"
        subdomains='abcd'
      ...
      <Marker position={[14.07266545382338, 100.60213663812874]} icon={new L.Icon({ iconUrl: markerIcon, iconSize: [32, 32] })}>
        <Popup>
